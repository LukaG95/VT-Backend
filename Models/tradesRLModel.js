...
TradeRL.collection.dropIndex({ bumpedAt: 1 }, function (err, result) {
...
exports.validateTrade = async (trade, user, req) => {
        ...
        Slot.Items.map((item) => {
            if (item.Tradable) {
                ...
                for (
                    let i = 0;
                    i < trade.have.length;
                    i++ // this checks if itemID and itemName are related
                )
                    if (trade.have[i].itemID === item.ItemID) if (trade.have[i].itemName === item.Name) checker++;

                for (let i = 0; i < trade.want.length; i++)
                    if (trade.want[i].itemID === item.ItemID) if (trade.want[i].itemName === item.Name) checker++;
            }
        })
    );
    if (checker !== trade.want.length + trade.have.length)
        return { error: { details: [{ message: "itemID doesn't match with itemName" }] } };
    ...
    const hwValidation = Joi.object({
        ...
        }) // limit 100000 if credits are the item
    });
