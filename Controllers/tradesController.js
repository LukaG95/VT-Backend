...
exports.getTrades = async (req, res, next) => {
    ...
    const advancedQuery = new AdvancedQueryRL(TradeRL.find(), query).filter().paginate().sortByLatest();
    ...
    return res
        .status(200)
        .json({ info: "success", message: "successfully got trades", trades: readableActiveAt(trades), pages });
};
...
exports.getUserTrades = async (req, res, next) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(searchId))
        return res.status(400).json({ info: "searchId", message: "Invalid searchId" });

    const trades = await TradeRL.find({ user: searchId }).populate("user").sort("-bumpedAt");
    if (trades.length < 1)
        return res.status(200).json({ info: "no trades", message: "user has no trades created", trades: [] });
...
exports.getTrade = async (req, res, next) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(tradeId))
        return res.status(400).json({ info: "tradeId", message: "Invalid tradeId" });
...
exports.editTrade = async (req, res, next) => {
    ...
    if (trade.user.toHexString() !== user._id.toHexString())
        return res.status(401).json({ info: "forbidden", message: "can't edit others trades" });
...
exports.bumpTrade = async (req, res, next) => {
    ...
    if (!trade) return res.status(404).json({ info: "no trade", message: "trade with given id doesn't exist" });
    if (trade.user.toHexString() !== user._id.toHexString())
        return res.status(401).json({ info: "unauthorized", message: "can't bump others trades" });
...
exports.deleteTrade = async (req, res, next) => {
    ...
    if (!trade) return res.status(404).json({ info: "no trade", message: "trade with given id doesn't exist" });
    if (trade.user.toHexString() !== user._id.toHexString())
        return res.status(401).json({ info: "unauthorized", message: "can't delete others trades" });
