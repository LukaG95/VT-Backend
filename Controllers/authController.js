...
const createToken = (id, code = 0, email) =>
    jwt.sign({ id, code, email }, process.env.JWT_SECRET, {
...
const createSendToken = (user, res, option) => {
    ...
    const cookieSettings = {
        expires: new Date(Date.now() + expires * 86400 * 1000),
        httpOnly: true
    };
...
exports.getUserByUsername = async (req, res, next) => {
    ...
    if (user.length < 1)
        return res.status(400).json({ info: "no user", message: `user was not found by the name of ${username}` });
...
exports.adminOnly = async (req, res, next) => {
    ...
    if (user.role !== "admin")
        return res
            .status(403)
            .json({ info: "forbidden", message: "looks like you don't have the permission to access this floor" });
...
exports.resetPassword = catchAsync(async (req, res, next) => {
    ...
    if (
        (await user.compareTokens(decodedCode.code, user.verificationToken)) &&
...
async function sendSignupEmail(user) {
    ...
    const Email = new EmailingSystem({ email: user.email }).sendSignup(token);
    await Email;
}
...
async function sendPasswordResetEmail(user) {
    ...
    await user.save();
    const token = await createToken(user._id, emailToken);
    const Email = new EmailingSystem({ email: user.email }).sendPasswordReset(token);
    await Email;
}
...
async function sendEmailUpdateEmail(user, newEmail) {
    ...
    const Email = new EmailingSystem({ email: user.email }).sendEmailUpdate(token);
    await Email;
}
