...
exports.getReputation = async (req, res, next) => {
    ...
    const rep = await Reputation.aggregate([
        ...
        {
            $addFields: {
                // add these fields just as count, later not adding them in $project
                csgoReps: {
                    $filter: {
                        input: "$csgoReps",
                        as: "rep",
...
exports.addReputation = async (req, res, next) => {
    ...
    if (!receiving_user)
        return res.status(404).json({ info: "no user", message: "user with the given id does not exist" });

    if (receiving_user._id.toHexString() === user._id.toHexString())
        return res.status(400).json({ info: "rep yourself", message: "you can not rep yourself" });
...
exports.getTop10 = async (req, res, next) => {
    ...
    const aggregation = function (Date) {
        ...
        if (Date) match = { "reps.createdAt": { $gte: Date } };
